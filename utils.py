...
WORK_DIR = file_path[
    : len(file_path) - file_path[::-1].find("/")
]  # TODO: fix for no ./
...
PRIME_TYPING_CHANGE_ACTIVATION = [
    c
    for c in lines[0][
        lines[0].find("'") + 1 : len(lines[0]) - lines[0][::-1].find("'") - 1
    ]
]
...
# apple scripts:
CHANGE_LANGUAGE_SCRIPT = (
    'osascript -e \'tell application "System Events" to tell process "SystemUIServer" to '
...
GET_ACTIVE_WINDOW_SCRIPT = (
    'osascript -e \'tell app "System Events" to get name of first process whose frontmost is '
    "true' "
)
...
def get_current_layout():
    ...
    return current_layout[
        current_layout.find(LAYOUT_RETURN_PREFIX)
        + len(LAYOUT_RETURN_PREFIX) : current_layout.find(LAYOUT_RETURN_POSTFIX)
    ]
...
def convert_typing(layout, stack):
    ...
    if layout != MAIN_LAYOUT and layout != SECONDARY_LAYOUT:
        ...
        return [
            CONVERSION_PAIRS[layout][key] if key in CONVERSION_PAIRS[layout] else key
            for key in stack
        ]


SECONDARY_TYPING_CHANGE_ACTIVATION = convert_typing(
    MAIN_LAYOUT, PRIME_TYPING_CHANGE_ACTIVATION
)
